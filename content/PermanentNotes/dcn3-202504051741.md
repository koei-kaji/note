...
WezTerm では、設定時点で自分がしたい Pane 分割ができないため、 Pane 分割には tmux を使用する。
...
設定をしていうくときに毎回 WezTerm の再起動は面倒なので、Auto reload が行われるようにする。
...
Pane 分割に tmux を使っているため、Terminal 起動時に tmux が起動するように設定する。
